...
namespace QdtJXCConnector.WCFListenService.Listening
{
    ...
    public class KRightListen : IBaseListen<T_Right>, IDisposable
    {
        ...
        public void InitListen(List<GraspcwZT> ls)
        {
            try
            {
                foreach (var zt in ls)
                {
                    ...
                    var sqlTableDependency = new SqlTableDependency<T_Right>(con, "T_KRight");
        ...
        {
            TRightIn right = new TRightIn(entity, 2);
            right.DogNumber = GraspcwZTDao.GetDogNumberByTableName(dbName);
            right.DbName = dbName;
            BaseInput input = new BaseInput(right);
